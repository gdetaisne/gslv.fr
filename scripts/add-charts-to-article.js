...
console.log('📊 Ajout de graphiques à l\'article');
console.log('==================================\n');

// Article avec graphiques intégrés
const articleWithCharts = [
  {
    id: "fr-cfo-1",
...
![Graphique des causes d'échec des startups](https://via.placeholder.com/600x300/1e40af/ffffff?text=73%+des+startups+échouent+à+cause+de+problèmes+financiers)
...
![Graphique du reporting financier](https://via.placeholder.com/500x250/059669/ffffff?text=68%+des+PME+n'ont+pas+de+reporting+en+temps+réel)
...
![Graphique des échecs de levées de fonds](https://via.placeholder.com/500x250/dc2626/ffffff?text=85%+des+levées+échouent+sans+préparation)
...
![Graphique des causes de faillite](https://via.placeholder.com/500x250/7c3aed/ffffff?text=42%+des+startups+font+faillite+pour+problèmes+de+trésorerie)
...
![Graphique des ratios LTV/CAC](https://via.placeholder.com/500x250/ea580c/ffffff?text=60%+ont+un+ratio+LTV/CAC+<+3)
...
![Graphique des coûts CFO](https://via.placeholder.com/500x250/0891b2/ffffff?text=CFO+part-time+coûte+3x+moins+cher+qu'un+CDI)
...
![Timeline des 4 étapes](https://via.placeholder.com/600x200/1e40af/ffffff?text=Étape+1:+Audit+→+Étape+2:+Structuration+→+Étape+3:+Optimisation+→+Étape+4:+Transfert)
...
![Graphique des erreurs courantes](https://via.placeholder.com/500x300/6b7280/ffffff?text=Attendre+trop+longtemps+→+Négliger+formation+→+Mesurer+trop+tard+→+Communication+insuffisante)
...
![Graphique de conclusion](https://via.placeholder.com/600x200/059669/ffffff?text=Startups+avec+CFO:+3x+plus+de+chances+de+réussir)
...
*Sources : CB Insights 2024, PwC Global SME Survey, McKinsey Global Institute, Ernst & Young Venture Capital Report, France Digitale, Startup Genome, INSEE*`,
    category: "cfo",
...
fs.writeFileSync(path.join(__dirname, '../data/blog/all.json'), JSON.stringify(articleWithCharts, null, 2));
fs.writeFileSync(path.join(__dirname, '../data/blog/fr.json'), JSON.stringify(articleWithCharts, null, 2));
fs.writeFileSync(path.join(__dirname, '../public/data/blog/all.json'), JSON.stringify(articleWithCharts, null, 2));
fs.writeFileSync(path.join(__dirname, '../public/data/blog/fr.json'), JSON.stringify(articleWithCharts, null, 2));

console.log('✅ Graphiques ajoutés à l\'article !');
console.log('📊 Graphiques intégrés :');
...
console.log('   - Causes de faillite (42%)');
console.log('   - Ratios LTV/CAC (60%)');
console.log('   - Coûts CFO part-time vs CDI');
console.log('   - Timeline des 4 étapes');
console.log('   - Erreurs courantes');
console.log('   - Graphique de conclusion');
console.log('\n🎨 Article plus visuel et professionnel !');
